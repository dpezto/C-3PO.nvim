...
local function resolve(kind, value, where)
  ...
  local ok, mod = pcall(require, ("c3po.%s.%s"):format(kind, value))
  if not ok or type(mod) ~= "table" then
    vim.notify(("[c3po] %s: no such %s: %q"):format(where, kind, value), vim.log.levels.ERROR)
    return nil
  end
  return mod
end
...
---@return { [1]: c3po.ColorPicker, [2]: c3po.ColorOutput }[]
...
---@return table<c3po.ColorPicker, { [1]: c3po.ColorInput, [2]: c3po.ColorOutput }>
