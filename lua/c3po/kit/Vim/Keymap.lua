local kit = require("c3po.kit")
local Async = require("c3po.kit.Async")

---@alias c3po.kit.Vim.Keymap.Keys { keys: string, remap: boolean }
---@alias c3po.kit.Vim.Keymap.KeysSpecifier string|{ keys: string, remap: boolean }

---@param keys c3po.kit.Vim.Keymap.KeysSpecifier
---@return c3po.kit.Vim.Keymap.Keys
...
---@param callback fun()
---@return c3po.kit.Async.AsyncTask
function Keymap.next(callback)
  return Keymap.send(""):next(callback)
end

---Send keys.
---@param keys c3po.kit.Vim.Keymap.KeysSpecifier|c3po.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("c3po.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("c3po.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
